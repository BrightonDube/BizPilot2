"""Unit tests for AI model routing (no app dependencies).

These tests verify the core routing logic without importing the full app.
"""
...
# Import only the routing module
import sys
...
class TestModelRegistry:
    ...
    def test_reasoning_has_multiple_models(self):
        """Verify reasoning has multiple models for fallback."""
        assert len(MODEL_REGISTRY["reasoning"]) >= 2
...
class TestEnvironmentOverrides:
    ...
    def test_no_override_returns_registry_defaults(self):
        """Test that without overrides, registry defaults are used."""
        models = get_models_for_task(TaskType.FAST)
        assert models == MODEL_REGISTRY["fast"]
...
class TestGroqExecutionWrapper:
    ...
    @pytest.mark.asyncio
    async def test_run_groq_empty_response(self):
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            with patch("app.core.ai_models.settings") as mock_settings:
                ...
                with pytest.raises(ModelExecutionError, match="No content in response"):
                    await run_groq(
                        model="llama-3.1-8b-instant",
                        messages=[{"role": "user", "content": "test"}]
                    )
...
class TestModelSelectionLogic:
    ...
    def test_fallback_task_returns_fallback_models(self):
        """Verify fallback task type returns fallback models."""
        models = get_models_for_task(TaskType.FALLBACK)
        assert models == MODEL_REGISTRY["fallback"]
...
class TestNoHardcodedModels:
    ...
    def test_registry_models_are_strings(self):
        """Verify all model names in registry are strings."""
        for task_type, models in MODEL_REGISTRY.items():
            for model in models:
                assert isinstance(model, str), f"Model {model} in {task_type} is not a string"
                assert len(model) > 0, f"Empty model name in {task_type}"
...
class TestLoggingAndObservability:
    ...
    async def test_success_logging(self, caplog):
        ...
        with patch("app.core.ai_models.run_groq", new_callable=AsyncMock) as mock_groq:
            ...
            assert any("Success" in msg or "success" in msg for msg in log_messages)
    ...
    async def test_failure_logging(self, caplog):
        ...
        with patch("app.core.ai_models.run_groq", new_callable=AsyncMock) as mock_groq:
            ...
            log_messages = [record.message for record in caplog.records]
            assert any("failure" in msg.lower() or "failed" in msg.lower() for msg in log_messages)
