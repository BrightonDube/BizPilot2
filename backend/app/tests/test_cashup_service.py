...
async def test_generate_cashup_aggregates_all_shift_orders_correctly(db):
    ...
    (
        mock_totals.total_amount,
        mock_totals.total_tips,
        mock_totals.tables_served,
        mock_totals.cover_count,
    ) = Decimal("100.00"), Decimal("10.00"), 5, 12
    db.execute.side_effect = [
        AsyncMock(scalars=lambda: MagicMock(first=lambda: mock_shift)),
        AsyncMock(scalars=lambda: MagicMock(first=lambda: None)),
        AsyncMock(first=lambda: mock_totals),
        AsyncMock(
            all=lambda: [
                MagicMock(
                    method_type=PaymentMethodType.CARD, sum_amount=Decimal("110.00")
                )
            ]
        ),
    ]
    ...
    assert (
        result.total_sales == Decimal("100.00")
        and result.total_tips == Decimal("10.00")
        and result.card_collected == Decimal("110.00")
...
async def test_approve_cashup_changes_status(db):
    ...
    result = await CashupService.approve_cashup(
        mock_cashup.id, uuid.uuid4(), BIZ_ID, db
    )
...
async def test_reject_cashup_requires_reason(db):
    ...
    result = await CashupService.reject_cashup(
        mock_cashup.id,
        uuid.uuid4(),
        BIZ_ID,
        CashupRejectRequest(rejection_reason="Discrepancy in cash"),
        db,
    )
