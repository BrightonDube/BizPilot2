...
async def test_get_active_floor_plan_returns_all_tables_with_status(db):
    ...
    db.execute.side_effect = [
        AsyncMock(scalars=lambda: MagicMock(first=lambda: mock_fp)),
    ...
    assert (
        ...
        and result.tables[0].name == "T1"
        and result.tables[0].status == "available"
    )


@pytest.mark.asyncio
async def test_get_active_floor_plan_uses_redis_cache_on_second_request(
    db, redis_client
):
    mock_fp_data = (
        '{"id": "'
        ...
        + '", "business_id": "'
        + str(BIZ_ID)
        + '", "name": "Main", "is_active": true, "width_units": 100, "height_units": 100, "tables": [], "created_at": "2026-03-13T12:00:00Z", "updated_at": "2026-03-13T12:00:00Z"}'
    )
...
async def test_get_table_status_returns_only_status_fields(db):
    T1_ID = uuid.uuid4()
    db.execute.side_effect = [
        AsyncMock(scalars=lambda: MagicMock(all=lambda: [T1_ID])),
        AsyncMock(scalars=lambda: MagicMock(all=lambda: [])),
    ]
    result = await FloorPlanService.get_table_status_only(BIZ_ID, db)
    assert (
        len(result) == 1
        and result[0].table_id == T1_ID
        and result[0].status == "available"
    )
...
async def test_floor_plan_cache_invalidated_after_table_created(db, redis_client):
    db.execute.return_value = AsyncMock(
        scalars=lambda: MagicMock(first=lambda: FloorPlan(id=FP_ID, business_id=BIZ_ID))
    )
    await FloorPlanService.create_floor_plan_table(
        FP_ID,
        FloorPlanTableCreate(name="T2", x_position=30, y_position=40),
        BIZ_ID,
        db,
        redis_client,
    )
...
async def test_assign_waiter_to_section_replaces_existing_assignment(db):
    await FloorPlanService.assign_waiter_to_section(
        "Window", uuid.uuid4(), FP_ID, BIZ_ID, db
    )
