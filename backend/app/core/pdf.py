...
def build_invoice_pdf(
    ...
    # Bank Details section (if provided and balance due > 0)
    if (bank_name or bank_account_number) and float(balance_due) > 0:
        pdf.y_position -= 10
        pdf._set_color(0.95, 0.95, 0.95)
        pdf._add_rect(pdf.left_margin, pdf.y_position - 5, pdf.right_margin - pdf.left_margin, 50, fill=True, stroke=False)
        pdf._reset_color()
        
        pdf._set_color(0.2, 0.4, 0.8)
        pdf._add_text("PAYMENT DETAILS", pdf.left_margin + 5, 10, bold=True)
        pdf._reset_color()
        pdf.y_position -= 15
        
        if bank_name:
            pdf._add_text(f"Bank: {bank_name}", pdf.left_margin + 5, 9)
            pdf.y_position -= 12
        if bank_account_number:
            pdf._add_text(f"Account Number: {bank_account_number}", pdf.left_margin + 5, 9)
            pdf.y_position -= 12
        if bank_branch_code:
            pdf._add_text(f"Branch Code: {bank_branch_code}", pdf.left_margin + 5, 9)
            pdf.y_position -= 12
        
        pdf._add_text(f"Reference: {invoice_number}", pdf.left_margin + 5, 9)
        pdf.y_position -= 15
...
def build_purchase_order_pdf(
    ...
    # Order details
    order_number: str,
    order_date: date | datetime | None,
    # Ordered by (user who created the order)
    ordered_by_name: Optional[str],
    ordered_by_email: Optional[str],
    ordered_by_phone: Optional[str],
    # Items
    items: list[dict],
    subtotal: Decimal,
    tax_amount: Decimal,
    ...
    """Build a professionally styled purchase order PDF.
    
    This PDF includes:
    - Business (buyer) details with contact info
    - Supplier details with contact info
    - Order details with items table
    - Ordered by information (user who created the order)
    - Reply-to email for easy communication
    """
    pdf = PDFBuilder(title=f"Purchase Order {order_number}")
    
    # Header with business name
    pdf.add_header(business_name, "Purchase Order")
    
    # PO Number and Date - right aligned
    pdf._add_text("PURCHASE ORDER", pdf.right_margin - 120, 18, bold=True)
    pdf.y_position -= 20
    pdf._add_text(order_number, pdf.right_margin - 120, 12)
    pdf.y_position -= 15
    pdf._set_color(0.4, 0.4, 0.4)
    pdf._add_text(f"Date: {format_date(order_date)}", pdf.right_margin - 120, 10)
    pdf._reset_color()
    pdf.y_position -= 30
    
    # Two column layout: From (Business) | To (Supplier)
    start_y = pdf.y_position
    
    # Left column - FROM (Business/Buyer)
    pdf._set_color(0.2, 0.4, 0.8)
    pdf._add_text("FROM:", pdf.left_margin, 10, bold=True)
    pdf._reset_color()
    pdf.y_position -= 15
    ...
    if business_email:
        pdf._add_text(f"Email: {business_email}", pdf.left_margin, 9)
        pdf.y_position -= 11
        
    if business_vat:
        pdf._add_text(f"VAT: {business_vat}", pdf.left_margin, 9)
        pdf.y_position -= 11
    
    left_end_y = pdf.y_position
    
    # Right column - TO (Supplier)
    pdf.y_position = start_y
    
    pdf._set_color(0.2, 0.4, 0.8)
    pdf._add_text("TO:", 320, 10, bold=True)
    pdf._reset_color()
    pdf.y_position -= 15
    
    pdf._add_text(supplier_name, 320, 10, bold=True)
    pdf.y_position -= 12
    
    if supplier_contact:
        pdf._add_text(f"Attn: {supplier_contact}", 320, 9)
        pdf.y_position -= 11
    
    if supplier_address:
        for line in supplier_address.split("\n")[:4]:
            pdf._add_text(line, 320, 9)
            pdf.y_position -= 11
    
    if supplier_phone:
        pdf._add_text(f"Tel: {supplier_phone}", 320, 9)
        pdf.y_position -= 11
    
    if supplier_email:
        pdf._add_text(f"Email: {supplier_email}", 320, 9)
        pdf.y_position -= 11
    ...
    # Ordered By section
    if ordered_by_name or ordered_by_email or ordered_by_phone:
        pdf._set_color(0.95, 0.95, 0.95)
        pdf._add_rect(pdf.left_margin, pdf.y_position - 5, pdf.right_margin - pdf.left_margin, 35, fill=True, stroke=False)
        pdf._reset_color()
        
        pdf._set_color(0.3, 0.3, 0.3)
        pdf._add_text("ORDERED BY:", pdf.left_margin + 5, 9, bold=True)
        pdf._reset_color()
        pdf.y_position -= 12
        
        order_by_parts = []
        if ordered_by_name:
            order_by_parts.append(ordered_by_name)
        if ordered_by_email:
            order_by_parts.append(f"Email: {ordered_by_email}")
        if ordered_by_phone:
            order_by_parts.append(f"Tel: {ordered_by_phone}")
        
        pdf._add_text(" | ".join(order_by_parts), pdf.left_margin + 5, 9)
        pdf.y_position -= 25
    ...
    # Subtotal
    pdf._add_text("Subtotal:", x_label, 10)
    pdf._add_text(format_currency(subtotal, currency), x_value, 10)
    pdf.y_position -= 15
    
    # Tax
    pdf._add_text("VAT (15%):", x_label, 10)
    pdf._add_text(format_currency(tax_amount, currency), x_value, 10)
    pdf.y_position -= 15
    
    # Total line
    pdf._set_color(0.2, 0.4, 0.8)
    pdf._add_line(x_label - 10, pdf.y_position + 5, pdf.right_margin, pdf.y_position + 5, 1)
    pdf._reset_color()
    pdf.y_position -= 5
    
    # Total
    pdf._add_text("TOTAL:", x_label, 12, bold=True)
    pdf._add_text(format_currency(total, currency), x_value, 12, bold=True)
    pdf.y_position -= 30
    
    # Notes section
    if notes:
        pdf._set_color(0.3, 0.3, 0.3)
        pdf._add_text("Notes:", pdf.left_margin, 10, bold=True)
        pdf._reset_color()
        pdf.y_position -= 15
        
        for line in notes.split("\n")[:5]:
            pdf._add_text(line[:80], pdf.left_margin, 9)
            pdf.y_position -= 12
        pdf.y_position -= 10
    ...
    if business_email:
        pdf.y_position -= 10
        pdf._set_color(0.4, 0.4, 0.4)
        pdf._add_text(f"For queries, please reply to: {business_email}", pdf.left_margin, 9)
        pdf._reset_color()
