...
async def check_manager_role(
    ...
    business_id: str = Depends(deps.get_current_business_id),
    ...
    if (
        not business_user
        or not business_user.role
        or business_user.role.name.lower() not in ["admin", "manager"]
    ):
        raise HTTPException(
...
async def create_floor_plan(
    ...
    db: AsyncSession = Depends(deps.get_db),
    business_id: str = Depends(deps.get_current_business_id),
    current_user=Depends(check_manager_role),
):
...
async def update_floor_plan(
    ...
    return await FloorPlanService.update_floor_plan(
        floor_plan_id, data, UUID(business_id), db
    )
...
async def create_floor_plan_table(
    ...
    return await FloorPlanService.create_floor_plan_table(
        floor_plan_id, data, UUID(business_id), db, redis
    )
...
async def update_floor_plan_table(
    ...
    return await FloorPlanService.update_floor_plan_table(
        floor_plan_id, table_id, data, UUID(business_id), db, redis
    )


@router.delete(
    "/{floor_plan_id}/tables/{table_id}", status_code=status.HTTP_204_NO_CONTENT
)
async def delete_floor_plan_table(
    ...
    await FloorPlanService.delete_floor_plan_table(
        floor_plan_id, table_id, UUID(business_id), db, redis
    )


@router.post(
    "/{floor_plan_id}/sections/assign", response_model=SectionAssignmentResponse
)
async def assign_waiter_to_section(
    ...
    return await FloorPlanService.assign_waiter_to_section(
        section_name, waiter_id, floor_plan_id, UUID(business_id), db
    )
