...
class CashupService:
    ...
    async def generate_waiter_cashup(shift_id: UUID, waiter_id: UUID, business_id: UUID, db: AsyncSession) -> WaiterCashupResponse:
        ...
        if (await db.execute(select(WaiterCashup).filter(WaiterCashup.shift_id == shift_id))).scalars().first():
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Cashup already generated")
    ...
    async def approve_cashup(cashup_id: UUID, approved_by: UUID, business_id: UUID, db: AsyncSession) -> WaiterCashupResponse:
        ...
        if not cashup:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Cashup not found")
        if cashup.waiter_id == approved_by:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Cannot approve own cashup")
