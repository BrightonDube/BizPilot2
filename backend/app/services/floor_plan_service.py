...
class FloorPlanService:
    ...
    async def get_active_floor_plan(business_id: UUID, db: AsyncSession, redis_client: Optional[Redis] = None) -> FloorPlanResponse:
        ...
        if not floor_plan:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No active floor plan found")
    ...
    async def get_table_status_only(business_id: UUID, db: AsyncSession, redis_client: Optional[Redis] = None) -> List[TableStatusResponse]:
        ...
        if redis_client:
            cached = await redis_client.get(cache_key)
            if cached:
                return [TableStatusResponse.model_validate(item) for item in json.loads(cached)]
        ...
        if redis_client:
            await redis_client.setex(cache_key, 10, json.dumps([s.model_dump(mode='json') for s in status_list]))
        return status_list
    ...
    async def create_floor_plan_table(floor_plan_id: UUID, data: FloorPlanTableCreate, business_id: UUID, db: AsyncSession, redis_client: Optional[Redis] = None) -> FloorPlanTableResponse:
        if not (await db.execute(select(FloorPlan).filter(FloorPlan.id == floor_plan_id, FloorPlan.business_id == business_id))).scalars().first():
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Floor plan not found")
