...
async def get_weekly_report(
    ...
    return svc.get_weekly_report(UUID(business_id), parsed)
...
async def get_monthly_report(
    ...
    from uuid import UUID
    svc = SalesReportService(db)
    return svc.get_monthly_report(UUID(business_id), year, month)
...
async def get_product_performance(
    ...
    svc = SalesReportService(db)
    return svc.get_product_performance(UUID(business_id), start, end, limit=limit)
...
async def get_dashboard_kpis(db: Session, user: User) -> Dict[str, Any]:
    ...
    return ai_svc.build_business_context(user, AIDataSharingLevel.METRICS_ONLY)
