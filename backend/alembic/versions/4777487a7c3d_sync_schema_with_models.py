...
def upgrade() -> None:
    ...
    # Create foreign keys - these are safe to call (will fail silently if exists)
    try:
    ...
    # These alter_column operations change VARCHAR to ENUM - skip if already done
    # They are safe to re-run if the column is already the target type
    try:
