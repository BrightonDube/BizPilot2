...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('payment_method', postgresql.ENUM('cash', 'card', 'bank_transfer', 'mobile', 'check', 'other', name='paymentmethod', create_type=False), default='cash'),
        sa.Column('status', postgresql.ENUM('pending', 'completed', 'failed', 'refunded', 'cancelled', name='paymentstatus', create_type=False), default='pending'),
