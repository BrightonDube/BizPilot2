...
def run():
    ...
    try:
        ...
        result = db.execute(text("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'subscription_tiers');")).scalar()
        print(f"Table 'subscription_tiers' exists: {result}")
