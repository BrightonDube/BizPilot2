    ...
    async function fetchData() {
      try {
        ...
        const [categoryRes, categoriesRes] = await Promise.all([
          ...
          apiClient.get<{ items: Category[] }>('/categories?limit=100'),
        ])

        const category = categoryRes.data
        // Filter out the current category from parent options to prevent circular reference
        setCategories((categoriesRes.data.items || []).filter(c => c.id !== categoryId))
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-400">Loading category...</p>
        </div>
      </div>
    )
  }
