...
export default function NewInvoicePage() {
  ...
  useEffect(() => {
    async function loadCustomers() {
      try {
        const response = await apiClient.get('/customers?per_page=100')
    ...
    async function loadProducts() {
      try {
        const response = await apiClient.get('/products?per_page=100')
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-400 mt-1">Create a new invoice for your customer</p>
          </div>
        </div>
      </div>
